...
public class RadMechAI : EnemyAI, IVisibleThreat
{
	...
	public override void FinishedCurrentSearchRoutine()
	{
		...
		if (currentBehaviourStateIndex == 0 && base.IsServer && !Physics.Raycast(base.transform.position + Vector3.up * 9.62f, Vector3.up, 60f, StartOfRound.Instance.collidersAndRoomMaskAndDefault, QueryTriggerInteraction.Ignore))
		{
	...
	public override void Update()
	{
		...
		case 0:
			...
			if (!base.IsServer && currentBehaviourStateIndex != previousBehaviour && previousBehaviourStateIndex == 2)
			{
	...
	private void Stomp(Transform stompTransform, ParticleSystem particle, ParticleSystem particle2 = null, float radius = 5f)
	{
		...
		if (num < radius)
		{
			...
			if (!(localPlayerController.physicsParent != null))
			{
