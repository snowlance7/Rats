...
public class DoublewingAI : EnemyAI
{
	...
	public void SetFlyDirection()
	{
		...
		if (daytimeEnemyLeaving)
		{
			...
			{
				if (Body.position - previousPosition == Vector3.zero)
				{
					Body.rotation = Quaternion.Lerp(Body.rotation, Quaternion.Euler(0f, base.transform.eulerAngles.y, 0f), 5f * Time.deltaTime);
				}
				else
				{
