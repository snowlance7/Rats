...
public class QuickMenuManager : MonoBehaviour
{
	...
	public CanvasGroup BGPanelTransparency;
	...
	public GameObject specialOption;
	...
	private void Start()
	{
		...
		if (specialOption != null)
		{
			...
			{
				specialOption.SetActive(value: true);
			}
			else
			{
				specialOption.SetActive(value: false);
			}
		}
	...
	public void Debug_SetWeedCount(string numString)
	{
		if (!UnityEngine.Object.FindObjectOfType<QuickMenuManager>().CanEnableDebugMenu() || !NetworkManager.Singleton.IsConnectedClient || !NetworkManager.Singleton.IsServer)
		{
			return;
		}
		numString = Regex.Replace(numString, "[^.0-9]", "");
		int num = Convert.ToInt32(numString);
		if (num > -1 && num < StartOfRound.Instance.allPlayerScripts.Length)
		{
			CadaverGrowthAI cadaverGrowthAI = UnityEngine.Object.FindObjectOfType<CadaverGrowthAI>();
			if (cadaverGrowthAI != null)
			{
				cadaverGrowthAI.InfectPlayer(StartOfRound.Instance.allPlayerScripts[num], severe: true, emittingSpores: true);
				cadaverGrowthAI.InfectPlayerRpc(num, severe: true);
			}
		}
	}
	...
	{
		if (CanEnableDebugMenu() && NetworkManager.Singleton.IsConnectedClient && NetworkManager.Singleton.IsServer)
		{
			GameNetworkManager.Instance.localPlayerController.KillPlayer(Vector3.zero);
		}
	}

	public void Debug_CadaverBloomBurstPlayer()
	{
		if (CanEnableDebugMenu() && NetworkManager.Singleton.IsConnectedClient && NetworkManager.Singleton.IsServer)
		{
			CadaverGrowthAI cadaverGrowthAI = UnityEngine.Object.FindObjectOfType<CadaverGrowthAI>();
			if (cadaverGrowthAI != null)
			{
				cadaverGrowthAI.InfectPlayer(StartOfRound.Instance.allPlayerScripts[0], severe: true);
				cadaverGrowthAI.InfectPlayerRpc(0, severe: true);
			}
			StartOfRound.Instance.Debug_SyncCadaverBloomBurstClientRpc();
		}
	}
	...
	public void Debug_SwitchIsInsideFactory()
	{
		if (CanEnableDebugMenu())
		{
			GameNetworkManager.Instance.localPlayerController.isInsideFactory = !GameNetworkManager.Instance.localPlayerController.isInsideFactory;
			StartOfRound.Instance.SwitchIsInsideFactoryRpc((int)GameNetworkManager.Instance.localPlayerController.playerClientId, GameNetworkManager.Instance.localPlayerController.isInsideFactory);
		}
	}
	...
	public bool CanEnableDebugMenu()
	{
