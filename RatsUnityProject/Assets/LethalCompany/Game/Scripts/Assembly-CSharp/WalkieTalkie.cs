...
public class WalkieTalkie : GrabbableObject
{
	...
	{
		if (StartOfRound.Instance.currentLevel.levelID == 3 && isBeingUsed)
		{
			if (!wallAudio.isPlaying)
			{
				wallAudio.Play();
				wallAudio.pitch = UnityEngine.Random.Range(0.92f, 1.08f);
			}
			float num = Vector3.Distance(base.transform.position, new Vector3(-38.51f, -7.72f, -10.56f));
			wallAudio.volume = math.remap(22f, 0f, 0f, 1f, Mathf.Min(num, 22f));
			if (!(Time.realtimeSinceStartup - wallAudioCheckInterval > 20f))
			{
				return;
			}
			if (wallAudio.clip == wallAudios[1])
			{
				wallAudio.clip = wallAudios[0];
				return;
			}
			wallAudioCheckInterval = Time.realtimeSinceStartup;
			if (num < 5f && UnityEngine.Random.Range(0, 120) < 3)
			{
				wallAudio.clip = wallAudios[1];
				PlayWallAudioRpc();
			}
		}
		else
