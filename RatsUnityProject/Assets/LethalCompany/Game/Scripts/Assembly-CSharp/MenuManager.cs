...
public class MenuManager : MonoBehaviour
{
	...
	public TextMeshProUGUI HoverTipText;
	...
	private void Start()
	{
		...
		{
			DisplayMenuNotification("Could not connect to Steam servers! (If you just want to play on your local network, choose LAN on launch.)", "Continue", setSelected: false);
			defaultValue = "Unnamed";
		}
		else
		{
		...
		if (!flag)
		{
			...
			else if (GameNetworkManager.Instance.disableSteam)
			{
				EventSystem.current.SetSelectedGameObject(lanWarningContainer.gameObject.GetComponentInChildren<Button>().gameObject);
			}
			else
			{
	...
	public void SetSelectedUIToNextNotificationPanel()
	{
		if (GameNetworkManager.Instance.disableSteam && lanWarningContainer.gameObject.activeSelf)
		{
			EventSystem.current.SetSelectedGameObject(lanWarningContainer.gameObject.GetComponentInChildren<Button>().gameObject);
		}
		else if (menuNotification.activeSelf)
		{
	...
	public void DisplayMenuNotification(string notificationText, string buttonText, bool setSelected = true)
	{
		if (!isInitScene)
		{
