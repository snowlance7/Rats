...
public class ForestGiantAI : EnemyAI, IVisibleThreat
{
	...
	{
		base.Start();
		playerStealthMeters = new float[4];
	...
	{
		base.Update();
		if (GameNetworkManager.Instance.localPlayerController == null || timeSinceSpawn < exitVentAnimationTime - 1.6f)
		{
			return;
		}
		if (isEnemyDead)
	...
	{
		if (GetAllPlayersInLineOfSightNonAlloc(50f, 70, eye, 3f, StartOfRound.Instance.collidersRoomDefaultAndFoliage) > 0)
		{
			PlayerControllerB[] tempPlayersArray = RoundManager.Instance.tempPlayersArray;
			PlayerControllerB playerControllerB = tempPlayersArray[0];
			int num = 0;
			float num2 = 1000f;
			PlayerControllerB playerControllerB2 = tempPlayersArray[0];
			float num3 = 0f;
			float num4 = 1f;
			for (int i = 0; i < StartOfRound.Instance.allPlayerScripts.Length; i++)
			{
				if (tempPlayersArray.Contains(StartOfRound.Instance.allPlayerScripts[i]))
				{
					float num5 = Vector3.Distance(StartOfRound.Instance.allPlayerScripts[i].transform.position, eye.position);
					if (num5 > 16f && StartOfRound.Instance.allPlayerScripts[i].isUnderwater)
					{
						playerStealthMeters[i] -= Time.deltaTime * 0.2f;
						continue;
					}
					...
					if (num4 >= 2f || num5 < 66f || (num4 >= 1f && num5 < 45f))
					{
						playerStealthMeters[i] = Mathf.Clamp(playerStealthMeters[i] + Time.deltaTime / (num5 * 0.21f) * scrutiny * num4, 0f, 1f);
					}
			...
			if (currentBehaviourStateIndex == 1)
			{
				if (lostPlayerInChase)
				{
					chasingPlayerInLOS = playerStealthMeters[chasingPlayer.playerClientId] > 0.35f;
					return;
				}
				chasingPlayerInLOS = tempPlayersArray.Contains(chasingPlayer);
				if (stunnedByPlayer != null)
				{
					playerControllerB = stunnedByPlayer;
				}
				if (playerControllerB != chasingPlayer && playerStealthMeters[num] > 0.35f && timeSinceChangingTarget > 2f)
				{
			...
			if (currentBehaviourStateIndex != 1 && timeSpentStaring > 3f && !investigating)
			{
				investigating = true;
				hasBegunInvestigating = false;
				investigatePosition = RoundManager.Instance.GetNavMeshPosition(playerControllerB2.transform.position, default(NavMeshHit), 5f, agentMask);
			}
		}
		else
		{
			...
			timeSpentStaring = 0f;
			for (int j = 0; j < playerStealthMeters.Length; j++)
			{
				playerStealthMeters[j] = Mathf.Clamp(playerStealthMeters[j] - Time.deltaTime * 0.33f, 0f, 1f);
			}
		}
	}
