...
public class AnimatedObjectTrigger : NetworkBehaviour
{
	...
	public void Start()
	{
		...
		if (doorType != -1)
		{
			...
			{
			case 0:
				boolTrueAudios = SoundManager.Instance.steelDoorOpenSFX;
				boolFalseAudios = SoundManager.Instance.steelDoorCloseSFX;
				break;
			case 1:
				boolTrueAudios = SoundManager.Instance.woodDoorOpenSFX;
				boolFalseAudios = SoundManager.Instance.woodDoorCloseSFX;
				break;
			}
		}
	}
	...
	private void PlayAudio(bool boolVal, bool playSecondaryAudios = false)
	{
		...
		else if (boolFalseAudios.Length != 0)
		{
			if (playAudiosInSequence)
			{
				...
				if (timesTriggered == playParticleOnTimesTriggered && playParticle != null)
				{
					playParticle.Play(withChildren: true);
