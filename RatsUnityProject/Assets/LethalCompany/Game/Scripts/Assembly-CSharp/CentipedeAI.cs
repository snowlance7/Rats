...
public class CentipedeAI : EnemyAI
{
	...
	public static List<CentipedeAI> allCentipedes;

	public EnemyAICollisionDetect colliderScript;

	public override void Start()
	{
		mainEntrancePosition = RoundManager.FindMainEntrancePosition();
		offsetTargetPos = base.transform.position;
		if (allCentipedes == null)
		{
			allCentipedes = new List<CentipedeAI>(enemyType.MaxCount + 1);
		}
		if (!allCentipedes.Contains(this))
		{
			allCentipedes.Add(this);
		}
		base.Start();
	}
	...
	public override void OnDestroy()
	{
		...
		if (allCentipedes.Contains(this))
		{
			allCentipedes.Remove(this);
		}
	}

	public bool TargetClosestPlayerCentipede(float bufferDistance = 1.5f, bool requireLineOfSight = false, float viewWidth = 70f, bool doGroundCast = false, bool requirePath = false, bool checkForMineshaftStartTile = true)
	{
		...
		for (int i = 0; i < StartOfRound.Instance.connectedPlayersAmount + 1; i++)
		{
			...
			bool flag = false;
			for (int j = 0; j < allCentipedes.Count; j++)
			{
				if (allCentipedes[j].clingingToPlayer == StartOfRound.Instance.allPlayerScripts[i])
				{
					flag = true;
					break;
				}
			...
			if (doGroundCast)
			{
				if (!Physics.Raycast(StartOfRound.Instance.allPlayerScripts[i].transform.position, Vector3.down, out rayHit, 5f, StartOfRound.Instance.collidersAndRoomMaskAndDefault, QueryTriggerInteraction.Ignore) || (requirePath && PathIsIntersectedByLineOfSight(rayHit.point, calculatePathDistance: false, avoidLineOfSight: false)))
				{
					continue;
				}
			}
	...
	public override void DoAIInterval()
	{
		...
		else if (!TargetClosestPlayerCentipede())
		{
			...
			else if (!clingingToCeiling && PathIsIntersectedByLineOfSight(destination, calculatePathDistance: false, avoidLineOfSight: false))
			{
				choseHidingSpotNoPlayersNearby = false;
			}
	...
	public override void Update()
	{
		...
		case 2:
			...
			colliderScript.onlyCollideWhenGrounded = true;
			break;
		case 3:
	...
	private void IncreaseSpeedSlowly(float increaseSpeed = 1.5f)
	{
		...
		else
		{
			...
			agent.speed = Mathf.Clamp(agent.speed + Time.deltaTime * 1.5f, 0f, 5.2f);
		}
	}
	...
	private void StopClingingToPlayer(bool playerDead)
	{
		if (clingingToPlayer == null)
		{
			Debug.LogError($"Centipede attempted to stop clinging to a player but was not clinging to anyone. playerDead: {playerDead}");
			return;
		}
	...
	public override void OnCollideWithPlayer(Collider other)
	{
		...
		for (int i = 0; i < allCentipedes.Count; i++)
		{
			if (allCentipedes[i].clingingToPlayer == playerControllerB)
			{
				return;
			}
		}
	...
	private IEnumerator fallFromCeiling()
	{
		...
		else
		{
			...
			startPosition = RoundManager.Instance.GetNavMeshPosition(ray.GetPoint(4f), default(NavMeshHit), 10f);
	...
	private IEnumerator clingToCeiling()
	{
		...
		colliderScript.onlyCollideWhenGrounded = false;
		ceilingAnimationCoroutine = null;
		startedCeilingAnimationCoroutine = false;
	}
