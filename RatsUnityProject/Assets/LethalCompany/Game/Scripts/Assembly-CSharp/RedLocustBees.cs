...
public class RedLocustBees : EnemyAI
{
	...
	private PlayerControllerB[] shuffledPlayersArray;
	...
	{
		base.Start();
		shuffledPlayersArray = StartOfRound.Instance.allPlayerScripts.ToArray();
		ShufflePlayersArray(shuffledPlayersArray);
		if (base.IsServer)
		{
			SpawnHiveNearEnemy();
			syncedLastKnownHivePosition = true;
		}
		timeAtSpawn = Time.realtimeSinceStartup;
	}

	private void ShufflePlayersArray(PlayerControllerB[] playersArray)
	{
		System.Random random = new System.Random(StartOfRound.Instance.randomMapSeed + 2);
		for (int num = playersArray.Length - 1; num > 0; num--)
		{
			int num2 = random.Next(0, num + 1);
			PlayerControllerB playerControllerB = playersArray[num];
			playersArray[num] = playersArray[num2];
			playersArray[num2] = playerControllerB;
		}
	}
	...
	private void SpawnHiveNearEnemy()
	{
		if (base.IsServer)
		{
			...
			int layerMask = agentMask & -17;
			Vector3 randomNavMeshPositionInBoxPredictable = RoundManager.Instance.GetRandomNavMeshPositionInBoxPredictable(base.transform.position, 10f, RoundManager.Instance.navHit, random, layerMask);
	...
	public PlayerControllerB CheckLineOfSightForPlayerBees(float width = 45f, int range = 60, int proximityAwareness = -1)
	{
		...
		float num = 1000f;
		for (int i = 0; i < shuffledPlayersArray.Length; i++)
		{
			if (!shuffledPlayersArray[i].isUnderwater)
			{
				Vector3 position = shuffledPlayersArray[i].gameplayCamera.transform.position;
	...
	public override void DoAIInterval()
	{
		...
		case 0:
		{
			...
			bool flag2 = Time.realtimeSinceStartup - timeAtSpawn < 5f;
			if (Vector3.Distance(base.transform.position, lastKnownHivePosition) > 2f || flag2)
			{
		...
		case 1:
			if (targetPlayer == null || !PlayerIsTargetable(targetPlayer) || targetPlayer.isUnderwater || Vector3.Distance(targetPlayer.transform.position, hive.transform.position) > (float)defenseDistance + 5f)
			{
				targetPlayer = null;
				wasInChase = false;
				if (IsHiveMissing())
		...
		case 2:
		{
			if (IsHivePlacedAndInLOS())
			{
				...
				if (playerControllerB != null && Vector3.Distance(playerControllerB.transform.position, hive.transform.position) < (float)defenseDistance && !playerControllerB.isUnderwater)
				{
	...
	{
		int allPlayersInLineOfSightNonAlloc = GetAllPlayersInLineOfSightNonAlloc(360f, 16);
		PlayerControllerB playerControllerB = null;
		if (allPlayersInLineOfSightNonAlloc > 0)
		{
			...
			{
				PlayerControllerB playerControllerB2 = RoundManager.Instance.tempPlayersArray[i];
				if (playerControllerB2.isUnderwater)
				{
					continue;
				}
				if (playerControllerB2.currentlyHeldObjectServer != null)
				{
					if (num3 == -1 && playerControllerB2.currentlyHeldObjectServer.itemProperties.itemId == 1531)
					{
						num3 = i;
						continue;
					}
					if (playerControllerB2.currentlyHeldObjectServer == hive)
					{
						return playerControllerB2;
					}
				}
				if (targetPlayer == null)
				{
					float num4 = Vector3.Distance(base.transform.position, playerControllerB2.transform.position);
					if (num4 < num)
					{
						num = num4;
						num2 = i;
			...
			if (num3 != -1 && Vector3.Distance(base.transform.position, RoundManager.Instance.tempPlayersArray[num3].transform.position) - num > 7f)
			{
				playerControllerB = RoundManager.Instance.tempPlayersArray[num2];
			}
			else if (playerControllerB == null)
			{
				return RoundManager.Instance.tempPlayersArray[num2];
			}
		}
		...
		case 0:
			...
			if (!overrideBeeParticleTarget)
			{
				if (hive != null)
				{
					...
					if (num2 < 2f || (num2 < 5f && !Physics.Linecast(eye.position, hive.transform.position + Vector3.up * 0.5f, StartOfRound.Instance.collidersAndRoomMaskAndDefault, QueryTriggerInteraction.Ignore)))
					{
