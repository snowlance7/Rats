...
public class OccludeAudio : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (checkInterval >= 0.5f)
		{
			...
			if ((base.transform.position - StartOfRound.Instance.audioListener.transform.position).sqrMagnitude < Mathf.Min(thisAudio.maxDistance * thisAudio.maxDistance, 12100f))
			{
