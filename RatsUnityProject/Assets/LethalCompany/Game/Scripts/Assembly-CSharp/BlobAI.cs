...
public class BlobAI : EnemyAI
{
	...
	public override void Update()
	{
		...
		for (int j = 0; j < SlimeBonePositions.Length; j++)
		{
			if (Vector3.Distance(centerPoint.position, SlimeBonePositions[j]) > distanceOfRaysLastFrame[j])
			{
				SlimeBones[j].transform.position = Vector3.Lerp(SlimeBones[j].position, SlimeBonePositions[j], 10f * Time.deltaTime);
			}
			else
			{
				SlimeBones[j].transform.position = Vector3.Lerp(SlimeBones[j].position, SlimeBonePositions[j], 5f * Time.deltaTime);
			}
		}
	}
