...
public class AnimatedItem : GrabbableObject
{
	...
	public bool disableAnimationsWhenDropped;
	...
	public override void Start()
	{
		...
		if (disableAnimationsWhenDropped)
		{
	...
	public override void DiscardItem()
	{
		...
		if (!makeAnimationWhenDropping)
		{
			...
			if (disableAnimationsWhenDropped)
			{
				...
				discardCoroutine = StartCoroutine(waitForDiscardAnimationToFinish());
			}
			return;
		}
		if (itemRandomChance.Next(0, 100) < chanceToTriggerAnimation)
	...
	private IEnumerator waitForDiscardAnimationToFinish()
	{
		float timeAtStart = Time.realtimeSinceStartup;
		while (Time.realtimeSinceStartup - timeAtStart > 7f)
		{
			yield return null;
			if (!itemAnimator.IsInTransition(0) && itemAnimator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.99f)
			{
				break;
			}
		}
